...
class ObjectFraneworkTest: XCTestCase {
    ...
    func testPopulateData() {
        ...
        for (index, a) in Address.populate().enumerate() {
            ...
            XCTAssertTrue(trace[index] == "INSERT OR REPLACE INTO Address ( createdAt, updatedAt, street, number, state, zip ) VALUES ('\(a.createdAt!.datatypeValue)', '\(a.updatedAt!.datatypeValue)', '\(a.street)', \(a.number), '\(a.state)', \(a.zip))", "it isn't compatible")
        }
        /*
         Save Object
        ...
        */
        ...
        User.transaction {
            ...
            user.gender    = "male"
            let cal = NSCalendar.currentCalendar()
            user.birthday = cal.dateByAddingUnit(.Day, value: -Int(arc4random_uniform(30)), toDate: NSDate(), options: [])
            user.birthday = cal.dateByAddingUnit(.Month, value: -Int(arc4random_uniform(12)), toDate: user.birthday!, options: [])
            user.birthday = cal.dateByAddingUnit(.Year, value: -user.age, toDate: user.birthday!, options: [])
        ...
        XCTAssertTrue(trace[1] == "INSERT OR REPLACE INTO Address ( createdAt, updatedAt, street, number, state, zip ) VALUES ('\(address.createdAt!.datatypeValue)', '\(address.updatedAt!.datatypeValue)', '\(address.street)', \(address.number), '\(address.state)', \(address.zip))", "it isn't compatible")
        XCTAssertTrue(trace[2] == "INSERT OR REPLACE INTO User ( createdAt, updatedAt, firstName, lastName, age, birthday, gender, email, registerNumber, Address_objectId ) VALUES ('\(user.createdAt!.datatypeValue)', '\(user.updatedAt!.datatypeValue)', '\(user.firstName)', '\(user.lastName!)', \(user.age), '\(user.birthday!.datatypeValue)', '\(user.gender!)', '\(user.email)', \(user.registerNumber), \(user.address.objectId!))", "it isn't compatible")
    ...
    func testPopulateDataOptional() {
        ...
        for i in 0 ... 30  {
            ...
            XCTAssertTrue(trace[i] == "INSERT OR REPLACE INTO TestFieldOptional ( createdAt, updatedAt, optionalString, optionalNSString, optionalNSInteger, optionalDate, optionalNumber, optionalData ) VALUES ('\(test.createdAt!.datatypeValue)', '\(test.updatedAt!.datatypeValue)', \(convertNil(test.optionalString)), \(convertNil(test.optionalNSString)), \(convertNil(test.optionalNSInteger)), \(convertNil(test.optionalDate)), \(convertNil(test.optionalNumber)), \(convertNil(test.optionalData)))")
        }
         ...
         */
        trace = []
        for (index, t) in testFieldOptionalList.enumerate() {
            
            t.optionalString = "Ok"
            t.optionalNSString = "NString OK"
            t.optionalNSInteger = 10
            t.optionalDate = NSDate()
            t.optionalNumber = nil
            t.optionalData = String("Test Data Optional Ok").dataUsingEncoding(NSUTF8StringEncoding)!
